...
EXCEL_FILE_CANDIDATES = ["MBM applicability.xlsx", "MBM-applicability.xlsx", "Task___Country_and_Sector.xlsx"]

TECH_NAME_MAP_EXCEL_TO_APP = {
...
MECH_COLS_EXCEL = ["ETS","Carbon Tax","Fuel Mandate","CfD","CCfD","CBAM","CORSIA","IMO Levy","VCM","CDM/PACM","AMC","Feebate"]

def _find_excel_file():
    for name in EXCEL_FILE_CANDIDATES:
        p = Path(name)
        if p.exists():
            return p
    return None

def load_country_data_from_excel_runtime():
    p = _find_excel_file()
    if p is None:
        return None
    try:
        df = pd.read_excel(p, sheet_name="Country Level")
    except Exception:
        return None
    df.columns = [str(c).strip() for c in df.columns]
    need = ["ISO3","Country","Region","Technology"]
    ...
    for _, row in df.iterrows():
        ...
        tech_raw = str(row.get("Technology", "")).strip()
        if not country or country == 'nan':
            continue
        tech = TECH_NAME_MAP_EXCEL_TO_APP.get(tech_raw, tech_raw)
        if country not in out:
            out[country] = {"region": region, "iso3": iso3, "techs": {}}
        if tech not in out[country]["techs"]:
            out[country]["techs"][tech] = {}
        for col in MECH_COLS_EXCEL:
            if col in df.columns:
                v = row.get(col, None)
                if pd.notna(v):
                    s = str(v).strip()
                    if s in ("D", "I"):
                        key = "VCM/CDM" if col in ("VCM", "CDM/PACM") else col
                        prev = out[country]["techs"][tech].get(key)
                        if prev != "D":
                            out[country]["techs"][tech][key] = s if prev is None else ("D" if "D" in (prev, s) else "I")
...
COUNTRY_DATA_RUNTIME = load_country_data_from_excel_runtime()
if COUNTRY_DATA_RUNTIME:
    COUNTRY_DATA_RAW = COUNTRY_DATA_RUNTIME
